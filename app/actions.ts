...
export async function assignNurseToSection(nurseId: string, sectionId: string) {
  if (isLocalMode()) {
    const db = readDb()
  ...
  const supabase = createClient()
  const { error } = await supabase.from('nurses').update({ section_id: sectionId }).eq('id', nurseId)
